...
async function fetchClasses() {
  ...
  try {
    ...
    if (error instanceof BackendApiError && error.status === 404) {
      errorMessage.value = 'Classes unavailable.'
    } else {
      errorMessage.value = error instanceof Error ? error.message : 'Unable to load classes.'
    }
  } finally {
...
<template>
  <article class="rounded-3xl border border-white/10 bg-white/6 p-6">
    <p class="text-sm uppercase tracking-[0.2em] text-slate-400">School widget</p>
    <h3 class="mt-3 text-xl font-semibold text-white">Classes</h3>
    <div
      v-if="isLoading"
    ...
    <div v-else class="mt-4 grid gap-3 sm:grid-cols-2">
      <div class="rounded-2xl border border-white/10 bg-black/10 px-4 py-3">
        <p class="text-xs uppercase tracking-[0.2em] text-slate-500">Active classes</p>
        <p class="mt-2 text-2xl font-semibold text-white">{{ totalClasses }}</p>
      </div>
      <div class="rounded-2xl border border-white/10 bg-black/10 px-4 py-3">
        <p class="text-xs uppercase tracking-[0.2em] text-slate-500">Students covered</p>
        <p class="mt-2 text-2xl font-semibold text-white">{{ totalStudents }}</p>
      </div>
    </div>
    <p class="mt-4 text-sm leading-6 text-slate-300">
      This widget appears only when the runtime config enables the `classes` feature for the school
      product.
    </p>
    <RouterLink
      to="/classes"
